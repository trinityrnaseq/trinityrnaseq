...
#ifdef MPI_ENABLED
    string rank_readsToTranscriptsOutputFile = bString + "/readsToComponents.out." + rankstring;
    int rank_outFile =  open(rank_readsToTranscriptsOutputFile.c_str(), O_WRONLY | O_CREAT | O_TRUNC , 0666);
#endif
...
#ifdef MPI_ENABLED 
        while((num_max_mem_reads%numranks)!=rank)
            {
           
                  vector<string> readSeqVector_looping;
                  vector<string> readNameVector_looping;
                ...
                for(int i=0;i<max_mem_reads;i++)
                    {
                        ...
                        if(!fastaReader.NextToVector(readSeqVector_looping, readNameVector_looping)) 
                            {
                                count =0;
                                break;
                            }
...
#ifdef MPI_ENABLED
           #pragma omp critical
            if(write(rank_outFile,buf.c_str(),buf.size())<0)
            {
                cerr <<  "error writing file "  << rank_readsToTranscriptsOutputFile  << ": " <<  strerror(errno)  << endl;
                exit(0);
            }
    ...
    for(int i=0;i<numranks;i++)
        {
            ...
            loop_ss << i;
            string loopstring = loop_ss.str();
            string loop_readsToTranscriptsOutputFile = bString + "/readsToComponents.out." + loopstring;
            s = s +  loop_readsToTranscriptsOutputFile + " ";
        }

    s = s + ">" + readsToTranscriptsOutputFile; 
    cerr<<"S = "<<s<<endl;
    timer_start();
    system(s.c_str());
    cerr<<"Time taken in writing the combined file = "<<timer_stop()<< " (in milliseconds)"<<endl;  
       }
