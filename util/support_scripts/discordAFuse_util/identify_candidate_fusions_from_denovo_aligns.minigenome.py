...
usage = "\n\n\tusage: " + sys.argv[0] + " chimera.trans.align.gff3\n\n"
...
def main():
    ...
    trans_to_gene_pairs = parse_trans_to_gene_pairs(chimera_trans_align_gff3)
    ...
    for trans_acc in trans_to_gene_pairs:
        ...
        #print "\t".join(str(x) for x in [trans_acc, gene_1_name, gene_1_orient, gene_2_name, gene_2_orient]); continue

        if gene_1_name == gene_2_name: continue
...
def parse_trans_to_gene_pairs(chimera_gff3):
    ...
    for line in open(chimera_gff3):
        ...
        gene_name = gene_contig_acc.replace("__", " ").rstrip().split(" ")[-1]
